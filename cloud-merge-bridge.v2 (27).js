/*! cloud-merge-bridge.v2.js (consolidated)
   Source of truth: Supabase row app_state(id=WORKSPACE_ID).
   - Boot: ALWAYS fetch cloud first, adopt to local & memory, then render login.
   - Save: wait for stable state; strip ephemerals; MERGE by default; REPLACE if destructive.
   - Poll: every 3s, adopt if serverTs advanced.
   - Exposes: window.forceAdopt() for manual sync; window.lastServerTs for debugging.
*/
(function(){
  ...
  if (!window.supabase && !window.supa){ WARN('Supabase client lib missing'); return; }
  const supa = window.supa || window.supabase;
  const client = (window.supaClient ||= supa.createClient(SUPA_URL, SUPA_KEY));
  ...
  function adoptRemote(remote, reason){
    ...
    try {
      window.state = deepClone(remote.state);
  ...
  async function flush(){
    ...
    try{
      ...
      if (!tasksValid(snap.tasks)){
        WARN('skip merge: tasks invalid/undefined');
        return;
      }
      ...
      // Mirror to local after successful cloud write
  ...
  // Expose save wrapper if not present
  if (typeof window.save === 'function'){
    ...
    // Provide a minimal save() that just flushes current state
  ...
  window.forceAdopt = async function(){
    ...
    if (remote && remote.state){
      window.lastServerTs = remote.updated_at || null;
      adoptRemote(remote, 'adopted cloud (force)');
    }
  };

  // Boot: ALWAYS hydrate from cloud first
