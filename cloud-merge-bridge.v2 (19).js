/*! cloud-merge-bridge.v2.js  (no-auth-in-cloud)
   Full-replace cloud writer + robust boot + coalesced saves (latest snapshot wins).
   Does NOT persist currentUserId to cloud; restores it from session on adopt.
*/
(function(){
  ...
  function normalizeState(src, fallbackLocal){
    ...
    // Do NOT trust cloud for login; assign from session or null
    try{
      const sess = sessionStorage.getItem(SESS) || '';
      s.currentUserId = sess ? sess : null;
  ...
  (function wrapSave(){
    ...
    async function flush(){
      ...
      // Strip auth (do not persist login to cloud)
      delete snap.currentUserId;

      // Mirror to local with session user preserved
      try{
        const sess = sessionStorage.getItem(SESS) || '';
        const toLocal = deepClone(snap);
        toLocal.currentUserId = sess ? sess : null;
    ...
    window.save = function(){
      ...
      saveTimer = setTimeout(() => { inflight = inflight.then(flush).catch(()=>{}); }, 700);
    };
  })();
