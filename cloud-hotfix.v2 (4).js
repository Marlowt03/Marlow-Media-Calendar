/*! cloud-hotfix.v2.js  (v2.1)
   Disable legacy savers; auto-save on destructive and create flows.
*/
(function(){
  ...
  document.addEventListener('click', (e) => {
    ...
    if (txt && isCreateText(txt)) {
      // allow scheduler to mutate tasks/clients first, then save once
      setTimeout(() => queueSave('create/auto-schedule'), 750);
      return;
    }
    if (txt && isDestructiveText(txt)){
      setTimeout(() => queueSave('delete/complete', true), 350);
      return;
    }
  }, true);

  // also listen to <button aria-label="Create ...">
  document.addEventListener('click', (e) => {
    const el = e.target;
